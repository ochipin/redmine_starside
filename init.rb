...
module RedmineStarside
  class ViewHook < Redmine::Hook::ViewListener
    def view_layouts_base_html_head(context = {})
      stylesheet_link_tag('starside', plugin: 'redmine_starside') +
      javascript_include_tag('starside', plugin: 'redmine_starside')
    end
  end
end
...
Redmine::WikiFormatting::Macros.register do
  ...
  %i[tip note warning danger].each do |type|
    desc "#{type}バナーを表示する"
    macro type do |obj, args, text|
      config  = REMARKS_CONFIG[type]
      title   = args.first&.strip || type.to_s.capitalize
      content = if text.present?
        textilizable(text.strip, :object => obj)
      elsif args[1].present?
        "<p>#{ERB::Util.html_escape(args[1].strip)}</p>"
      else
        ''
      end

      <<~HTML.html_safe
        <aside class="remarks #{config[:css]}">
        <p class="title">#{config[:icon]}#{ERB::Util.html_escape(title)}</p>
        #{content}
        </aside>
      HTML
    end
  ...
  desc 'タブを表示する'
  macro :tabs do |obj, args, text|
    tab_names = args.map(&:strip)
    tab_contents = text.split('+++').map(&:strip)

    return ''.html_safe if tab_contents.empty?

    uid = "tab-#{rand(36**6).to_s(36)}"

    # タブボタン群
    buttons = tab_contents.each_with_index.map do |_content, i|
      title = tab_names[i] || "Tab #{i + 1}"
      tab_id = "#{uid}-#{i}"
      active = i == 0
      <<~BTN
        <li>
          <button id="#{tab_id}" type="button" role="tab"
                  aria-controls="#{tab_id}-panel"
                  aria-selected="#{active}"
                  tabindex="#{active ? 0 : -1}">#{ERB::Util.html_escape(title)}</button>
        </li>
      BTN
    end.join

    # タブパネル群
    panels = tab_contents.each_with_index.map do |content, i|
      tab_id = "#{uid}-#{i}"
      active = i == 0
      body = textilizable(content, :object => obj)
      <<~PANEL
        <div class="tab-panel" id="#{tab_id}-panel" role="tabpanel"
            aria-labelledby="#{tab_id}" tabindex="0" #{active ? '' : 'hidden'}>
          #{body}
        </div>
      PANEL
    end.join
    ...
    <<~HTML.html_safe
      <div class="tabview">
        <div class="wrapper">
          <ul class="tablist" role="tablist">#{buttons}</ul>
        </div>
        #{panels}
      </div>
    HTML
